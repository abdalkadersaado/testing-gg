        ...
        >
          <div class="d-flex align-items-center">
            <div>
              ...
              />
              <div v-else>
                <img v-if="post.user.media[0].collection_name === 'profile'" :src="post.user.media[0].src_url" alt="" />
                <img v-else :src="post.user.media[0].src_url" alt="" />
              </div>
              ...
              >
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <div class="w-100" v-if="post.sharing_post != false">
                      <button
                        v-if="post.shared_post.post"
                        type="button"
                        class="btn btn-primary"
                        data-bs-dismiss="modal"
                        @click="shareFromShareProfileToMyProfile(this.share)"
                      >
                        {{ $t("Sharing Now") }}
                      </button>
                      <button
                        ...
                        class="btn btn-primary"
                        data-bs-dismiss="modal"
                        @click="shareFromSharePageToMyProfile(this.share)"
                      >
                        {{ $t("Sharing Now") }}
                      </button>
                    </div>
                  ...
                  <div class="modal-footer">
                    <div class="w-100" v-if="post.sharing_post != false">
                      <button
                        ...
                        class="btn btn-primary"
                        data-bs-dismiss="modal"
                        @click="shareFromProfileToMypage(this.share)"
                      >
                        {{ $t("Sharing Now") }}
                      </button>
                      <button
                        ...
                        class="btn btn-primary"
                        data-bs-dismiss="modal"
                        @click="shareFromPageToMypage(this.share)"
                      >
                        {{ $t("Sharing Now") }}
                      </button>
                    </div>
  ...
  methods: {
    ...
    getSharenumber(post) {
      ...
      if (post.sharing_post != false) {
        if (post.shared_post.post) {
          this.share = post.shared_post.post.id;
        }
        if (post.shared_post.post_page) {
          this.share = post.shared_post.post_page.id;
        }
      } else {
        this.share = post.id;
      }
    ...
    async shareFromProfileToMypage(id) {
      ...
      await shareServices
        .sharePersonalPostOnMyPage(formData)
        .then((response) => {
          this.content = null;
    ...
    async shareFromPageToMypage(id) {
      ...
      await shareServices.sharePagePostOnMyPage(formData).then((response) => {
        this.content = null;
        this.showSuccess({ message: "Shared successfully On My Page" });
        console.log(response);
      });
    ...
    async shareFromShareProfileToMyProfile(id) {
      ...
      await shareServices
        .sharePersonalPostOnMyProfile(formData)
        .then((response) => {
          this.content = null;
    ...
    async shareFromSharePageToMyProfile(id) {
      ...
      await shareServices
        .sharePagePostOnMyProfile(formData)
        .then((response) => {
          this.content = null;
          this.showSuccess({ message: "Shared successfully On My Profile" });
          console.log(response);
