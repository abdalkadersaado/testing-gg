<template>
    <div>
        <div class="mobile">
            <button class="btn btn-success" style="border-radius: 50%;" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasExample" aria-controls="offcanvasExample">
                <font-awesome-icon icon="plus" />
            </button>

            <div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
                <div class="offcanvas-header">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="28.616" viewBox="0 0 25 28.616">
                        <g id="pages-svgrepo-com" transform="translate(0.5)">
                            <path id="Path_113" data-name="Path 113" d="M18.769.5H6.115A3.626,3.626,0,0,0,2.5,4.115V20.385A3.626,3.626,0,0,0,6.115,24H18.769a3.626,3.626,0,0,0,3.615-3.615V4.115A3.626,3.626,0,0,0,18.769.5Z" transform="translate(1.615 0)" fill="#00a79d" stroke="#00a79d" stroke-linecap="round" stroke-linejoin="round" stroke-width="1"/>
                            <path id="Path_114" data-name="Path 114" d="M4.115,2.5A3.626,3.626,0,0,0,.5,6.115V20.577A5.326,5.326,0,0,0,5.923,26H16.769a3.626,3.626,0,0,0,3.615-3.615" transform="translate(0 1.615)" fill="none" stroke="#00a79d" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                        </g>
                    </svg>
                    <h5 class="offcanvas-title fw-bold" id="offcanvasExampleLabel">{{ $t("My Pages") }}</h5>
                    <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
            <div v-if="mypages.length > 0">
                <div v-for="(page, index ) in mypages" :key="index" :page="page" @click="goto(page.id)" class="page-box d-flex justify-content-between align-items-center">
                    <p class="fw-bold p-0 m-0">{{ page.name }}</p>
                    <img class="page-img" v-if="page.has_media_profile === false" src="@/assets/img_friends/default1.png" alt="">
                    <img class="page-img" v-else-if="page.media[0].collection_name === 'profile'" :src="page.media[0].src_url" alt="">
                    <img class="page-img" v-else-if="page.media[1].collection_name === 'profile'" :src="page.media[1].src_url" alt="">
                </div>
            </div>
            </div>
        </div>
        <div class="col-2 left" >
            <div class="row d-flex align-items-center mb-3" style="cursor: pointer;" @click="addpage()">
                <div class="col-md-12  left_box_info ">
                    <p class="fw-bold">{{ $t("Add New Page") }}</p>
                </div>
            </div>
            <div class="row mb-3 d-flex justify-content-between align-items-center">
                <div class="col-md-2 left_box_img">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="28.616" viewBox="0 0 25 28.616">
                        <g id="pages-svgrepo-com" transform="translate(0.5)">
                            <path id="Path_113" data-name="Path 113" d="M18.769.5H6.115A3.626,3.626,0,0,0,2.5,4.115V20.385A3.626,3.626,0,0,0,6.115,24H18.769a3.626,3.626,0,0,0,3.615-3.615V4.115A3.626,3.626,0,0,0,18.769.5Z" transform="translate(1.615 0)" fill="#00a79d" stroke="#00a79d" stroke-linecap="round" stroke-linejoin="round" stroke-width="1"/>
                            <path id="Path_114" data-name="Path 114" d="M4.115,2.5A3.626,3.626,0,0,0,.5,6.115V20.577A5.326,5.326,0,0,0,5.923,26H16.769a3.626,3.626,0,0,0,3.615-3.615" transform="translate(0 1.615)" fill="none" stroke="#00a79d" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                        </g>
                    </svg>
                </div>
                <div class="col-md-8 offset-lg-2  left_box_info ">
                    <p class="fw-bold p-0 m-0">{{ $t("My Pages") }}</p>
                </div>
            </div>

            <div v-if="mypages.length > 0" dir="auto">
               

                <div v-for="(page, index ) in mypages" :key="index" :page="page" @click="goto(page.id)" class="page-box d-flex justify-content-between align-items-center">
                     
                    <p class="fw-bold p-0 m-0"><router-link
                  class="router-link-to-profile"
                  :to="{
                    name: 'mypage',
                    params: { id: page.id },
                  }"
                  >{{ page.name }}</router-link></p>
                    <img class="page-img" v-if="page.has_media_profile === false" src="@/assets/img_friends/default1.png" alt="">
                    <img class="page-img" v-else-if="page.media[0].collection_name === 'profile'" :src="page.media[0].src_url" alt="">
                    <img class="page-img" v-else-if="page.media[1].collection_name === 'profile'" :src="page.media[1].src_url" alt="">
                </div>
            </div>

        </div>
    </div>
    
</template>

<script>
import { mapActions, mapState } from 'vuex';
export default {
    
    name: "LeftPageComponent",
    data() {
        return {
            // loading: null
        }
    },
    methods: {
        ...mapActions({
            getmypages : 'page/getmypages'
        }  
        ),
        goto(id) {
            this.$router.push({name:'mypage', params: {id: `${id}`}});
        },
        addpage() {
            this.$router.push({name:'addpage'});
        }
    },
    computed: {
        ...mapState(('page'),{
            mypages: "mypages"
        })
    },
    created() {
        this.getmypages()
        // console.log(this.mypages)
    }

}
</script>

<style lang="scss" scoped>
    .left {
        height: 100vh;
        // overflow-y:auto;
        // overflow-x:hidden;
        padding: 1rem 0.5rem ;
        width: 100%;


        .left_box_info {
            p {
                font-size: 20px !important;
            }
        }
        .page-box {
            margin:  3.5rem 0;
            padding: 0 1rem 0 0;
            cursor: pointer;

            .page-img {
                width: 50px;
                height: 50px;
                border-radius: 50%;
            }
        }
    }

    .mobile {
        display: none !important;
    }
    

    @media screen and (max-width: 992px) {
        .left {
            display: none !important;
        }
        .mobile {
            display: block !important;
            position: fixed !important;
            z-index: 99999 !important;
            top: 15px;
            left: 43%;
            

            #offcanvasExample {
            
                width: 100% !important;
                position: fixed;
                z-index: 222232 !important;
                top: 0px !important;
                left: 0 !important;
                overflow: scroll !important;

                .left_box_info {
                    p {
                        font-size: 20px !important;
                    }
                }
                .page-box {
                    margin:  1.5rem 0;
                    padding: 1rem 1rem 0 1rem !important;
                    cursor: pointer;

                    .page-img {
                        width: 50px;
                        height: 50px;
                        border-radius: 50%;
                    }
                }
                
            }
        }
    }

    // @media screen and (min-width: 1280px) and (max-width: 1600px){
    //     .left {
    //         background-color:#F3F3F3; 
    //         min-height: 100vh;
    //         padding: 2rem 1rem;

    //         .left_box {
    //             margin-bottom: 2rem;

    //             .left_box_img {
    //                 text-align: center;
    //             }

    //             .left_box_info {
    //                 margin-left: 2.1rem;
    //                 width: 60% !important;
    //                 font-size: 15px;
    //             }
    //         }
    //     }
    // }

    // @media screen and (min-width: 993px) and (max-width: 1279px){
    //     .left {
    //         // background-color:#F3F3F3; 
    //         min-height: 100vh;
    //         padding: 2rem 1rem;

    //         .left_box_info {
    //         p {
    //             font-size: 15px !important;
    //         }
    //     }
    //         .page-box {
    //         margin:  1.5rem 0 !important;
    //         // padding: 0 1rem 0 0;
    //         cursor: pointer;

    //         .page-img {
    //             width: 30px !important;
    //             height: 30px !important;
    //             border-radius: 50%;
    //         }
    //     }
    //     }
    // }



</style>
    
